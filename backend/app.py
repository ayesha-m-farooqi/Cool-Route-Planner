...
temp_lookup = routing_engine.TemperatureLookup("vegas_heatmap.geojson")
...
@app.route('/')
def health():
    return jsonify({"status": "Cool Route Planner API is running", "endpoints": ["/api/route", "/api/heatmap", "/api/cooling-stops"]})

@app.route('/api/heatmap', methods=['GET'])
def get_heatmap():
    """Return heatmap points [lat, lon, intensity] from the Vegas heatmap GeoJSON."""
    try:
        with open("vegas_heatmap.geojson", "r") as f:
            data = json.load(f)
        points = []
        for feat in data.get("features", []):
            ...
            # Polygon - use centroid approximation (first ring average)
...
def get_cooling_stops():
    ...
    try:
        with open("cooling_stops.json", "r") as f:
            stops = json.load(f)
...
def get_route():
    ...
    try:
        G = get_graph('walk') # Always use walk graph for demo robustness in small bounding box
        orig = ox.distance.nearest_nodes(G, start_lon, start_lat)
        dest = ox.distance.nearest_nodes(G, end_lon, end_lat)

        result = routing_engine.get_hot_and_cool_routes(G, orig, dest, alpha=5.0, mode=mode)
